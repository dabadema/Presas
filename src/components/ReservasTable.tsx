...
const ReservasTable: React.FC = () => {
  ...
  const fetchReservas = async (instalacionId: string) => {
    ...
    const response = await fetch(
      `http://localhost:3000/reservas/${instalacionId}`
    );
    const data = await response.json();
    if (response.ok) {
      setReservas(data);
  ...
  const obtenerReservaId = (hora: string, dia: string) => {
    for (const reserva of reservas) {
      ...
      const diaReserva = `${inicioReserva.getFullYear()}-${(
        inicioReserva.getMonth() + 1
      )
        .toString()
        .padStart(2, '0')}-${inicioReserva
        .getDate()
        .toString()
        .padStart(2, '0')}`;
      ...
      >
        ...
        {instalaciones.map((instalacion) => (
          <option
            ...
            value={instalacion.instalacionId}
          >
            {instalacion.nombre}
          </option>
        ))}
